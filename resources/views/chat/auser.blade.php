          ...
          <div class="messages">
            @foreach($messages as $message)
              @if ($message->user->role != "admin")
                @include('chat/broadcast', ["user" => $message->user, 'message' => $message->message])
              @else
            ...
            <div class="bottom">
              <form>
                <div class="input-group mb-3">
                  ...
                  <input type="hidden" name="chat_id" id="chat_id" value="{{ Auth::user()->id }}">
...
<script>
  ...
  const channel = pusher.subscribe('admin.{{ Auth::user()->id }}');
